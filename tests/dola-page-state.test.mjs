...
import {
  ...
  promptSignature,
} from '../dist-electron/dola-page-state.js'

test('detects Dola infringement and violation failures', () => {
  const text = '生成内容中疑似包含侵权 / 违规内容，无法返回该内容，换个主题再试试，生成额度未扣除。'
  const message = extractDolaFailureMessage(text)
...
test('detects common generation failure text', () => {
  assert.equal(extractDolaFailureMessage('视频生成失败，请稍后再试。'), '视频生成失败，请稍后再试。')
  assert.equal(extractDolaFailureMessage('你的视频免费额度还有 2 次。'), null)
})

test('detects the exhausted daily free generation quota message', () => {
  assert.equal(extractDolaFailureMessage('今日视频生成免费次数已用完。'), '今日视频生成免费次数已用完。')
  assert.equal(extractDolaFailureMessage('今日免费生成次数已用完，明天再来吧。'), '今日免费生成次数已用完，明天再来吧。')
  assert.equal(extractDolaFailureMessage('今日生成次数已用完。'), '今日生成次数已用完。')
  assert.equal(extractDolaFailureMessage('免费生成次数已用完，请明天再试。'), '免费生成次数已用完，请明天再试。')
...
test('accepts only new HTTP video links for the current generation', () => {
  ...
  assert.deepEqual(
    getNewDolaVideoUrls(['https://www.dola.com/thread/xbsMtOGRraAcehDc8'], []),
    []
  )
})
...
test('detects a newly submitted prompt in the conversation page', () => {
  ...
  const current = `${baseline} 用户 ${prompt} 助手 本次使用 Seedance 2.0 生成`
...
test('extracts current Dola share URL formats', () => {
  assert.equal(
    extractDolaShareUrl('复制链接：https://www.dola.com/thread/abc_123?from=share。'),
    'https://www.dola.com/thread/abc_123?from=share'
  )
  assert.equal(
    extractDolaShareUrl('https://dola.com/chat/chat_123#video'),
    'https://dola.com/chat/chat_123#video'
  )
  assert.equal(
    extractDolaShareUrl('https://www.dola.com/share/share-123'),
    'https://www.dola.com/share/share-123'
  )
  assert.equal(extractDolaShareUrl('https://example.com/thread/abc'), null)
})

test('accepts only chat pages as recoverable source conversations', () => {
  assert.equal(
    extractDolaConversationUrl('https://www.dola.com/chat/local_2452181702532277'),
    'https://www.dola.com/chat/local_2452181702532277'
  )
  assert.equal(
    extractDolaConversationUrl('https://www.dola.com/chat/38437129678594562'),
    'https://www.dola.com/chat/38437129678594562'
  )
  assert.equal(extractDolaConversationUrl('https://www.dola.com/thread/xZR7KqTbeRvEAjlB8'), null)
})

test('does not treat prompt rewrite pages as generated videos', () => {
  assert.equal(
    isDolaPromptRewritePage('完整 12 秒视频生成指令（可直接用于视频生成工具）'),
    true
  )
  assert.equal(
    isDolaPromptRewritePage('你的视频生成好了，点击分享即可复制链接。'),
    false
  )
})

test('detects the Dola desktop-download prompt that can block sharing', () => {
  assert.equal(
    isDolaDesktopDownloadPrompt('下载电脑版 使用完整功能 随时帮忙的 AI 桌面助手 下载电脑版 下次提醒我'),
    true
  )
  assert.equal(
    isDolaDesktopDownloadPrompt('下载电脑版 使用完整功能 下载电脑版'),
    false
  )
})

test('recognizes the completion text shown by Dola video cards', () => {
  assert.equal(isDolaGenerationComplete('你的视频生成好了。'), true)
  assert.equal(isDolaGenerationComplete('视频生成已提交，预计等待 5 分钟。'), false)
})
...
test('waits for a video element before treating the result as share-ready', () => {
  ...
  // The current Dola build renders a video poster as an image before the
